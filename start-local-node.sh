...
# Enable API server and swagger documentation
sed -i.bak '/^\[api\]/,/^\[/ { s/enable = false/enable = true/; s/swagger = false/swagger = true/; }' $HOME/.cronos/config/app.toml

# Enable telemetry
sed -i.bak '/^\[telemetry\]/,/^\[/ { s/enabled = false/enabled = true/; }' $HOME/.cronos/config/app.toml
