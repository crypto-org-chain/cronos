...
def setup_binary_compatibility_test_nix(tmp_path_factory, nix_package="binary-compat-package"):
    ...
    nodes_with_binary2 = [int(x.strip()) for x in nodes_with_binary2_str.split(",") if x.strip()]
            ...
            try:
                with open(log_file, "r") as f:
                    ...
                    patterns = [
                        ...
                        (r"wrong Block\.Header\.LastResultsHash", "Last results hash mismatch"),
                        (r"wrong Block\.Header\.DataHash", "Data hash mismatch"),
                        (r"wrong Block\.Header\.ValidatorsHash", "Validators hash mismatch"),
                        (r"wrong Block\.Header\.ConsensusHash", "Consensus hash mismatch"),
                        (r"CONSENSUS FAILURE", "Consensus failure"),
                        (r"consensus deems this block invalid", "Block deemed invalid by consensus"),
...
def get_compatibility_status(cronos, initial_heights, min_new_blocks=3, timeout=30):
    ...
    progression_results = check_block_progression(
        cronos, initial_heights, min_new_blocks=min_new_blocks, timeout=timeout, check_errors=False
    )
...
def display_compatibility_results(cronos, progression_results, consensus_errors, binary1_version, binary2_version):
    ...
    print(f"\n{'='*60}")
    print(f"Binary Compatibility Test Results")
    print(f"{'='*60}")
...
def check_for_breaking_change(cronos, binary1_version, binary2_version):
    ...
    if progressing_count < total_nodes:
        ...
        error_summary = "\n".join(error_details) if error_details else "  Check node logs for details"
        
        raise AssertionError(
            f"BREAKING CHANGE DETECTED during test execution:\n\n"
            f"Nodes stuck: {stuck_nodes}\n"
            f"Nodes progressing: {progressing_nodes if progressing_nodes else 'None'}\n\n"
            f"Binary versions:\n"
...
def check_block_progression(cronos, initial_heights, min_new_blocks=5, timeout=60, check_errors=True):
    """
    ...
    while time.time() - start_time < timeout:
        ...
        if check_errors and time.time() - last_error_check > 10:
            ...
            if error_nodes:
                ...
                raise AssertionError(
                    f"Consensus failure detected while waiting for blocks:\n" + 
                    "\n".join(error_msgs)
                )
...
def test_binary_compatibility(tmp_path_factory):
    ...
    for cronos, binaries in setup_binary_compatibility_test_nix(tmp_path_factory):
        ...
        print(f"\n{'='*60}")
        print(f"Testing Binary Compatibility")
        print(f"Expected: {'BREAKING' if expect_breaking else 'NON-BREAKING'}")
        print(f"Binaries from: configs/binary-compat-package.nix")
        print(f"{'='*60}\n")
        ...
        progressing_count, total_nodes = display_compatibility_results(
            cronos, progression_results, consensus_errors, binary1_version, binary2_version
        )
        ...
        if progressing_count < total_nodes:
            ...
            error_summary = "\n".join(error_details) if error_details else "  No specific error patterns found in logs (check node logs for details)"
        ...
        print(f"  The binaries are compatible and can work together\n")
