...
// mu guards base independently of Manager.stateMu: it is always the innermost
// lock (mempoolState never calls back into Manager while holding it), so
// nesting it under stateMu adds no ordering hazard, and base stays safe to
// read even if a future caller forgets to hold stateMu.
...
// the caller holds Manager.stateMu, which is what actually keeps this swap
...
// constructor, which leaves Manager.state nil, keeps working without a store.
