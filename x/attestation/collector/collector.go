...
func (c *BlockDataCollector) collectAndStoreBlock(ctx context.Context, eventData cmttypes.EventDataNewBlock) error {
	...
	attestationData := &types.BlockAttestationData{
		...
		Transactions:          c.encodeTransactions(block.Data.Txs),
		TxResults:             c.encodeTxResults(blockResults.TxsResults),
		FinalizeBlockEvents:   c.encodeFinalizeBlockEvents(blockResults.FinalizeBlockEvents),
