...
var (
	_ mempool.ExtMempool = (*PriorityNonceMempool[int64])(nil)
	_ mempool.Iterator   = (*PriorityNonceIterator[int64])(nil)
)

var (
	ErrWrongNonce = errors.New("tx nonce is incorrect")
)
...
func (mp *PriorityNonceMempool[C]) NextSenderTx(sender string) sdk.Tx {
	...
	return cursor.Value.(mempool.Tx).Tx
}
...
func (mp *PriorityNonceMempool[C]) InsertWithGasWanted(ctx context.Context, tx sdk.Tx, gasWanted uint64) error {
	...
	memTx := mempool.NewMempoolTx(tx, gasWanted)
	...
	if !ok {
		...
		// initialize sender next nonce
		mp.senderNextNonce[sender] = nonce
	}

	// Check sender next nonce
	if mp.senderNextNonce[sender] > nonce {
		return ErrWrongNonce
	} else if mp.senderNextNonce[sender] == nonce {
		mp.senderNextNonce[sender] = nonce + 1
	}
	...
	if oldScore, txExists := mp.scores[sk]; txExists {
		if mp.cfg.TxReplacement != nil && !mp.cfg.TxReplacement(oldScore.priority, priority, senderIndex.Get(key).Value.(mempool.Tx).Tx, tx) {
			return fmt.Errorf(
				"tx doesn't fit the replacement rule, oldPriority: %v, newPriority: %v, oldTx: %v, newTx: %v",
				oldScore.priority,
				priority,
				senderIndex.Get(key).Value.(mempool.Tx).Tx,
				tx,
			)
		}
...
func (i *PriorityNonceIterator[C]) iteratePriority() mempool.Iterator {
...
func (i *PriorityNonceIterator[C]) Next() mempool.Iterator {
	if i.priorityNode == nil {
		return nil
	}
...
func (i *PriorityNonceIterator[C]) Tx() mempool.Tx {
	return i.senderCursors[i.sender].Value.(mempool.Tx)
}
...
func (mp *PriorityNonceMempool[C]) Select(ctx context.Context, txs [][]byte) mempool.Iterator {
	mp.mtx.Lock()
	defer mp.mtx.Unlock()
	return mp.doSelect(ctx, txs)
}

func (mp *PriorityNonceMempool[C]) doSelect(_ context.Context, _ [][]byte) mempool.Iterator {
...
func (mp *PriorityNonceMempool[C]) SelectBy(ctx context.Context, txs [][]byte, callback func(mempool.Tx) bool) {
	mp.mtx.Lock()
	defer mp.mtx.Unlock()
...
func IsEmpty[C comparable](mempool mempool.Mempool) error {
