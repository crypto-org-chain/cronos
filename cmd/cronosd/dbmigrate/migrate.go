...
// Migrate performs a database migration from the source backend to the target backend
// according to the provided MigrateOptions. It creates a temporary target database
// (named "<dbName>.migrate-temp.db"), copies keys from the source to the target
// (optionally filtered by HeightRange when supported), and updates and returns a
// *MigrationStats containing totals, processed count, errors, and timing. If
// opts.Verify is true, a post-migration verification pass compares source and
// target key/value sets. The function may create the target data directory and
// open RocksDB when TargetBackend is RocksDBBackend. It does not automatically
// replace an existing target database; callers are responsible for renaming or
// moving the temporary directory to the final location. On success it returns a
// populated *MigrationStats and a nil error; on failure it returns the stats
// (partially populated) and a non-nil error.
...
// countKeys counts the number of keys present in the given database.
// It returns the total key count and any error encountered during iteration.
...
// countKeysWithHeightFilter counts keys in dbName that fall within heightRange using
// database-specific bounded iterators when supported; for unsupported databases it
// falls back to a full key count.
// It returns the number of keys that satisfy the height filter, or an error
// encountered while obtaining iterators or during iteration.
...
// migrateData migrates all key-value pairs from sourceDB to targetDB according to opts and updates stats.
// It performs a full (no height filtering) copy of entries and returns any error encountered during iteration or writing.
...
// migrateDataWithHeightFilter migrates keys from sourceDB to targetDB using iterators bounded by opts.HeightRange when the database type supports height filtering.
// It returns an error if iterator creation or migration fails; for unsupported database types it delegates to a full migration and ensures created iterators are closed.
...
// migrateWithIterator migrates key/value pairs from the provided iterator into the target database,
// applying height-range filtering from opts and committing data in batches sized by opts.BatchSize.
// It updates stats.ProcessedKeys and stats.ErrorCount, logs periodic progress to opts.Logger,
// and ensures keys/values are copied before batching to avoid iterator reuse issues.
// It returns any iterator error or an error encountered while writing batches.
...
// openDBWithRetry attempts to open the named database and retries with exponential backoff to handle transient OS-level file lock delays.
// It logs each failed attempt, doubles the delay between retries, and returns the opened DB on success or an error after maxRetries attempts.
...
// verifyMigration verifies that all keys that should be present in the target database exist and match the source, and that the target contains no extra keys.
// When opts.HeightRange is provided and supported for the database, verification is restricted to that height range. It returns an error describing any mismatches found.
