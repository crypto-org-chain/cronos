...
// PatchDatabase patches specific block heights from a source database into a target database
// according to the provided PatchOptions. It validates the options, opens the source and target
// databases, counts keys that match the configured height range, and applies height-filtered
// patches (respecting batch size, conflict resolution, and dry-run mode). RocksDB targets are
// flushed after a successful patch.
//
// It returns the MigrationStats containing progress and timing information, and a non-nil error
// if validation, database access, counting, or patching fails.
...
// countKeysForPatch counts keys that would be patched for the specified database and height range.
// It supports the blockstore and "tx_index" databases. If the provided HeightRange specifies
// particular heights, keys are filtered by extracting their heights and only keys within the
// range are counted. Iteration errors produce a returned error; unsupported dbName values
// also produce an error. The returned int64 is the total number of keys that match the filter.
...
// patchDataWithHeightFilter selects and runs the appropriate height-filtered patch routine for the configured database.
// It returns an error if the configured database is unsupported or if the selected patch routine fails.
...
// patchBlockstoreData copies blockstore keys from sourceDB into targetDB according to the provided height range and updates migration statistics.
// It obtains bounded iterators for relevant blockstore prefixes and processes each iterator with patchWithIterator; an error is returned if iterator retrieval or any iterator processing fails.
...
// patchTxIndexData patches tx_index entries from the source DB into the target DB,
// handling height-indexed tx.height keys, corresponding CometBFT txhash lookup keys,
// and ethereum event-indexed keys discovered in transaction results.
//
// The function iterates tx.height/<height>/ keys within the provided height range,
// applies conflict resolution (including prompting when configured), batches writes
// to the target DB, and collects CometBFT txhashes and ethereum tx info for
// subsequent passes. After processing tx.height keys it patches the txhash lookup
// entries and then scans and copies matching ethereum event-indexed keys from the
// source DB. Dry-run mode, batch sizing, and conflict strategy are driven by opts,
// and progress/metrics are recorded in stats.
//
// It returns an error if iterator creation, iterator traversal, batch writes, or
...
// patchTxHashKeys patches txhash lookup keys collected from the source into the target database.
// 
// It reads each provided txhash key from the source and writes the value to the target using batched writes,
// honoring the chosen conflict resolution strategy (skip, replace, or ask), supporting dry-run mode, and updating
// migration statistics for processed, skipped, and error counts. Missing source keys are ignored. Returns an error
// only on batch write failures.
...
// extractEthereumTxHash extracts the Ethereum transaction hash (0x-prefixed) from an ABCI TxResult protobuf value and returns it, or an empty string if no `ethereum_tx` event is present.
// It returns an error if the protobuf cannot be unmarshaled or if a found hash has an invalid length or contains non-hex characters.
...
// incrementBytes returns a new byte slice representing b interpreted as a big-endian unsigned integer plus one.
// The input slice is not modified; a copy with the incremented value is returned.
// If b is empty or all bytes are 0xFF (overflow), it returns nil to signal there is no greater exclusive upper bound.
...
//
// patchEthereumEventKeysFromSource patches ethereum event-indexed keys for the provided Ethereum transactions from the source DB into the target DB.
// 
// For each entry in ethTxInfos (where keys are 0x-prefixed Ethereum tx hashes and values are EthTxInfo containing Height and TxIndex),
// the function iterates a scoped prefix of the form `ethereum_tx.ethereumTxHash/<ethTxHash>/<height>/<txIndex>` in the source DB and copies matching
// keys (including those with optional `$es$<eventseq>` suffixes) into the target DB using batched writes. Conflict handling follows opts.ConflictStrategy
// and opts.SkipConflictChecks; in Ask mode event keys are treated as replace-to-avoid excessive prompting. When opts.DryRun is true, changes are logged but not written.
// The function updates stats.ProcessedKeys and stats.ErrorCount and logs processed/skipped counts.
//
// It returns an error if any iterator or batch write fails.
...
// patchWithIterator patches key-value pairs from the provided iterator into the target database,
// applying optional height-based filtering, conflict resolution, batched writes, and dry-run behavior.
//
// The function iterates over keys from `it`, skips keys outside `opts.HeightRange` when applicable,
// checks for existing keys in the target (unless `opts.SkipConflictChecks` is true) and resolves
// conflicts according to `opts.ConflictStrategy` (it may prompt the user when the strategy is
// `ConflictAsk`). Writes are collected into batches of size `opts.BatchSize` and flushed to the
// target (unless `opts.DryRun` is true). It increments `stats.ProcessedKeys` for written entries and
// increments `stats.ErrorCount` on I/O errors; it also logs progress and summaries during processing.
//
// Returns an error if the iterator reports an error, if a batch write fails, or if prompting for
...
// UpdateBlockStoreHeight updates the blockstore height metadata at the given targetPath to newHeight if newHeight is greater than the stored height.
// It opens the target database using the provided backend (using rocksDBOpts when the backend is RocksDB), preserves the existing Base value when present
// (defaults to 1), writes the updated BlockStoreState to the "BS:H" key, and flushes RocksDB if applicable. Returns an error if opening the database,
// reading or unmarshaling the existing state, marshaling the new state, writing the state, or flushing (for RocksDB) fails.
...
// promptKeyConflict prompts the user to resolve a conflicting key encountered during a patch
// operation and returns the user's decision.
//
// It prints a summary of the conflict (database, key, sizes, and when available the associated
// height) and reads a single-line response from standard input. The returned bool indicates
// whether the caller should write the new value (`true` = replace, `false` = skip). The
// returned ConflictResolution may update the overall strategy (for example `ConflictReplaceAll`
// or `ConflictSkip`) or remain `ConflictAsk` to continue prompting. An error is returned if
// reading input fails.
...
// formatKeyPrefix returns a concise, human-readable preview of the provided key,
// using a text representation when the key is mostly printable ASCII and a
// hexadecimal representation (prefixed with "0x") otherwise. The returned
// string will be truncated with "..." when it would exceed maxLen; an empty
// key yields "<empty>".
...
// returns a hex string prefixed with "0x", truncating with "... (N total bytes)" when the hex preview exceeds maxLen.
...
// formatStrategy converts a ConflictResolution value to a human-readable description.
// Possible outputs are "ask", "skip all", "replace", "replace all", and "unknown".
