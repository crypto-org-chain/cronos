...
# validate_backup_suffix validates a suffix contains only letters (A-Z, a-z), digits (0-9), period (.), underscore (_) and hyphen (-); prints an error and exits with status 1 if the suffix is empty or contains disallowed characters.
...
# usage displays help and usage information for the script, listing options and examples and then exits with status 1.
usage() {
    cat << EOF
Usage: $0 [OPTIONS]
...
# print_info prints an informational message prefixed with a blue [INFO] tag; the message is provided as the first argument.
print_info() {
    echo -e "${BLUE}[INFO]${NC} $1"
}

# print_success prints a green "[SUCCESS]" prefix followed by the given message to stdout.
print_success() {
    echo -e "${GREEN}[SUCCESS]${NC} $1"
}

# print_warning prints a warning message prefixed with `[WARNING]` in yellow to stdout.
print_warning() {
...
# get_size prints the disk usage size of the given directory (e.g., "4.0K") or prints "N/A" if the path does not exist or is not a directory.
get_size() {
    if [[ -d "$1" ]]; then
